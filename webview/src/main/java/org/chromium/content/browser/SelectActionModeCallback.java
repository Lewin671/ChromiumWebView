...
public class SelectActionModeCallback implements ActionMode.Callback {
    ...
    private void createActionMenu(ActionMode mode, Menu menu) {
        mode.getMenuInflater().inflate(R1.menu.select_action_menu, menu);
        if (!mEditable || !canPaste()) {
            menu.removeItem(R1.id.select_action_menu_paste);
        }

        if (!mEditable) {
            menu.removeItem(R1.id.select_action_menu_cut);
        }

        if (mEditable || !mActionHandler.isShareAvailable()) {
            menu.removeItem(R1.id.select_action_menu_share);
        }

        if (mEditable || mIncognito || !mActionHandler.isWebSearchAvailable()) {
            menu.removeItem(R1.id.select_action_menu_web_search);
        }
    }
    ...
    public boolean onActionItemClicked(ActionMode mode, MenuItem item) {
        ...
        if (id == R1.id.select_action_menu_select_all) {
            mActionHandler.selectAll();
        } else if (id == R1.id.select_action_menu_cut) {
            mActionHandler.cut();
        } else if (id == R1.id.select_action_menu_copy) {
            mActionHandler.copy();
            mode.finish();
        } else if (id == R1.id.select_action_menu_paste) {
            mActionHandler.paste();
        } else if (id == R1.id.select_action_menu_share) {
            mActionHandler.share();
            mode.finish();
        } else if (id == R1.id.select_action_menu_web_search) {
