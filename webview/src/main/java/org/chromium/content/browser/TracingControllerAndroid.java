...
public class TracingControllerAndroid {
    ...
    public boolean startTracing(boolean showToasts, String categories,
        ...
        if (!Environment.MEDIA_MOUNTED.equals(state)) {
            logAndToastError(
                    mContext.getString(R1.string.profiler_no_storage_toast));
            return false;
        }
    ...
    public boolean startTracing(String filename, boolean showToasts, String categories,
        ...
        if (!nativeStartTracing(mNativeTracingControllerAndroid, filename, categories,
                recordContinuously)) {
            logAndToastError(mContext.getString(R1.string.profiler_error_toast));
            return false;
        }

        logAndToastInfo(mContext.getString(R1.string.profiler_started_toast) + ": " + categories);
    ...
    protected void onTracingStopped() {
        ...
        logAndToastInfo(
                mContext.getString(R1.string.profiler_stopped_toast, mFilename));
        TraceEvent.setEnabledToMatchNative();
        mIsTracing = false;
        mFilename = null;
    }
