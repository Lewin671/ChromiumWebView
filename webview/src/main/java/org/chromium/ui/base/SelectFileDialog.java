...
class SelectFileDialog implements WindowAndroid.IntentCallback{
    ...
    private void selectFile(String[] fileTypes, boolean capture, WindowAndroid window) {
        ...
        if (captureCamera()) {
            if (window.showIntent(camera, this, R1.string.low_memory_error)) return;
        } else if (captureCamcorder()) {
            if (window.showIntent(camcorder, this, R1.string.low_memory_error)) return;
        } else if (captureMicrophone()) {
            if (window.showIntent(soundRecorder, this, R1.string.low_memory_error)) return;
        }
        ...
        if (!window.showIntent(chooser, this, R1.string.low_memory_error)) {
            onFileNotSelected();
        }
    }
    ...
    public void onIntentCompleted(WindowAndroid window, int resultCode,
        ...
        onFileNotSelected();
        window.showError(R1.string.opening_file_error);
    }
